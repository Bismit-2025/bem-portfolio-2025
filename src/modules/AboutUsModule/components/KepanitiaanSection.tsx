...
export default function KepanitiaanSection() {
  ...
  const handlePrev = () => {
    setActiveIndex(
      (prev) => (prev - 1 + committees.length) % committees.length,
    );
  };
  ...
  const getCardStyle = (index: number) => {
    ...
    const spacing =
      typeof window !== "undefined" && window.innerWidth < 768 ? 115 : 100;

    // Adapting logic from CircularGallery.tsx
    // x corresponds to horizontal offset in percentage
    const x = offset * spacing;

    // Tuning constants
    const bend = 30; // Controls the curvature height
    const H = 300; // The "horizon" width limit for the curve
    ...
    // translateY: arc is positive (distance from chord). We want a "hill" shape where center is highest.
    // In CSS translateY, 0 is top. Positive is down.
    // So center (arc=0) is 0. Sides (arc>0) move down.
    const translateY = arc * 2; // Multiplied for more dramatic effect if needed, or keeping raw arc. Let's try raw first.

    // rotateZ: Right side (x > 0) should tilt clockwise (positive deg).
    // Math.asin gives radians.
    const rotateRad = Math.sign(x) * Math.asin(effectiveX / R);
    ...
    return {
      transform: `perspective(1000px) translateX(${x}%) translateY(${translateY}px) rotateZ(${rotateDeg}deg) scale(${scale})`,
      opacity: isVisible ? opacity : 0,
      visibility: isVisible ? "visible" : "hidden",
      transition: "all 0.5s cubic-bezier(0.25, 0.8, 0.25, 1)",
      position: "absolute",
      zIndex: Math.round(100 - absOffset), // Ensure center card is on top
      left: "0",
      right: "0",
      margin: "0 auto",
    ...
    <section className="w-full py-12">
      <div className="container px-4 md:px-8 mx-auto flex flex-col items-center w-full overflow-hidden">
        ...
        <div className="relative w-full flex items-center justify-between md:justify-center mb-10 md:mb-16">
          <button
            onClick={handlePrev}
            className="md:hidden p-3 rounded-full bg-white border border-gray-200 shadow-md text-gray-600 active:scale-95 transition-transform z-20 shrink-0"
          >
            <svg
            ...
            {committees.map((item, index) => {
              ...
              return (
                ...
                >
                  {/* --- ISI KARTU --- */}
                  <div
                    className={`relative w-[400px] md:w-[450px] bg-white rounded-2xl overflow-hidden  ${activeIndex === index && "shadow-pacil-blue-300 shadow-xl"}  border border-gray-100 select-none h-[51w0px] md:h-[650px] transition-all duration-300`}
                  >
                    {/* Gambar Header */}
